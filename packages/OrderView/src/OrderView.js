...
import OrderListItem from '../../OrderList/src/OrderListItem';
import Accounting from '../../Checkout/src/Accounting';
import CartListProductItem from '../../CartList/src/CartListProductItem';
import {Subheader} from "../../List/src";
...
const OrderView = ({order, role}) => {
    ...
    if((order.status === ORDER_STATUS.completed || ORDER_STATUS_CANCELLED(order.status) === true) && (role === USER_ROLES.vendor || role === USER_ROLES.customer)) {
           ...
           {(role === USER_ROLES.vendor || role === USER_ROLES.customer)&& (
               <>
                ...
                <List.Section title="Products">
                    {order.products.map(data => <CartListProductItem data={data} />)}
                </List.Section>
               </>
           )}

            {fulfilmentDetails.length > 0 && <List.Section title={order.deliveryOption === PUB.delivery ? 'Delivery Details' : 'Pickup Details'}>
              {/*   {(order.status === ORDER_STATUS.completed || ORDER_STATUS_CANCELLED(order.status) === true) && (
                ...
                {fulfilmentDetails.map(item => <List.Item 
                        title={item.title} 
