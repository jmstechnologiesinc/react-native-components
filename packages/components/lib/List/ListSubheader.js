...
const ListSubheader = ({ title, overline, description, avatar }) => {
    const renderTitle = () => (
        <View>
            {overline && (
                <Text style={{ paddingHorizontal: 16 }} variant="labelSmall">
                    {overline}
                </Text>
            )}
    ...
    return (
        <List.Item
            ...
            left={
                avatar
                    ? () => <Avatar.Image size={40} style={{ margin: 8, marginRight: 0 }} source={{ uri: avatar }} />
                    : null
            }
        />
    );
